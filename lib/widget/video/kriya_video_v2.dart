...
class KriyaPlayerV2 extends StatefulWidget {
...
class _VidPlayerPage extends State<KriyaPlayerV2> with SingleTickerProviderStateMixin {
      ...
      child: PlayerWithControls(
        ...
        setFullScreen: () async {
          ...
          if (widget.videoBloc.interactive) {
            ...
            _interactiveBloc..add(FullscreenSwitchEvent(
    ...
    return BlocListener<VideoPlayerBloc, VideoPlayerState>(
      ...
      listener: (context, state) {
        if (state is VideoInitializedState) {
          if (state.end) {
            if (videoPlayerBloc.interactive) {
              print("end ${state.end} lastVideo ${state.lastVideo}");
              if (!state.lastVideo) {
                _interactiveBloc..add(NextVideoInteractiveEvent(interactive: state.interactive));
              }
            }
          }
          if (state.position >= Duration(seconds: 2)) {
            _interactiveBloc..add(LoadNextVideoEvent(interactive: state.interactive));
          }
        }
      },
                ...
                children: state.interactive.hotspots.map(
                  ...
                  ? Positioned(
                      ...
                      child: Container(
                        ...
                        child: InkWell(
                          onTap: () {
                            _interactiveBloc..add(OnClickHotspotEvent(hotspot: area, interactive: state.interactive));
                          },
                        ),
                      ),
                    )
        ...
        child: Center(
          child: GestureDetector(
            ...
            child: Container(
              ...
              height: 60,
              decoration: BoxDecoration(
                color: KriyaColor().kriyaLabFFFFFF,
                borderRadius: BorderRadius.circular(100.0),
              ),
              child: Center(
                child: Icon(
                  Icons.replay, 
                  color: KriyaColor().primaryColorThree,
                  size: 30.0,
                ),
              ),
            )
